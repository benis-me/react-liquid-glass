...
test("liquid work uses one internal smooth-union compositor for its full lifecycle", () => {
  ...
  assert.match(liquidCanvasSource, /float contour = 1\. - smoothstep\(0\., edgeWidth \* \.65, inside\)/);
  ...
  assert.match(liquidCanvasSource, /min\(\.85, edgeGain \* 3\.2\) \* mix\(1\., \.24, edgeLight\)/);
