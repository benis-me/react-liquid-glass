...
export function GlassVideo({ src, poster, caption, autoPlay = false, loop = true, muted = true, labels }: GlassVideoProps) {
  ...
  const materialRef = useRef(material); materialRef.current = material;
  ...
  useEffect(() => {
    ...
    const draw = (uploadVideo: boolean, now = performance.now()) => {
      ...
      for (let index = 0; index < 3; index += 1) {
        ...
        const [scale, velocity] = reduceRef.current ? [buttonTarget(index), 0] : stepSpring(
      ...
      strengthRef.current = reduceRef.current ? strengthTarget : Math.abs(strengthTarget - strengthRef.current) < 0.001 ? strengthTarget : strengthRef.current + (strengthTarget - strengthRef.current) * (1 - 0.82 ** (elapsed * 60));
      [barStretchRef.current, barStretchVelocityRef.current] = reduceRef.current ? [barStretchTargetRef.current, 0] : stepSpring(
      ...
      <div className="dg-video-demo__stage">
        ...
        >
          ...
          <div className="dg-video-player__controls" data-visible={controlsVisible && ready}>
            ...
            <div ref={barRef} className="dg-video-player__bar" role="slider" tabIndex={0} aria-label={text.progress} onKeyDown={event => {
              if (!["ArrowLeft", "ArrowRight", "Home", "End"].includes(event.key)) return;
              event.preventDefault(); const video = videoRef.current; if (!video || !Number.isFinite(video.duration)) return;
              if (event.key === "Home") video.currentTime = 0;
              else if (event.key === "End") video.currentTime = video.duration;
              else skip(event.key === "ArrowRight" ? 5 : -5);
              textureDirtyRef.current = true; ensureDrawRef.current();
            }} aria-valuemin={0} aria-valuenow={0} aria-valuemax={0} onPointerDown={startSeek} onPointerMove={moveSeek} onPointerUp={endSeek} onPointerCancel={endSeek} onLostPointerCapture={endSeek}>
